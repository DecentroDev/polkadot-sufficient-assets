...
const defaultConfig = {
  chains: [chains.polkadotAssetHubChain], // Polkadot Asset Hub will use its default WebSocket URL
};
```
...
export const libConfig = createConfig({
  chains: [chains.polkadotAssetHubChain], // Specify the chains
...
- **chains**: This defines the blockchain networks involved in the configuration. By default, the library connects to the network using WebSocket URLs. When light clients are enabled, the WebSocket connection is replaced by the Smoldot client.
