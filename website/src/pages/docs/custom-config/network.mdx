...
export const config = createConfig({
  ...
  xcmChains: [chains.hydration], // Optional cross-chain transfer configuration
});
```
...
- **chains**: The blockchain networks your application will interact with. In this case, the `polkadotAssetHubChain` has been selected.
- **xcmChains**: If cross-chain transfers (XCM) are enabled, you can specify the chains involved, such as `hydration`.
