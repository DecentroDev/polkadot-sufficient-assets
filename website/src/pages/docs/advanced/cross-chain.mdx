...
export const libConfig = createConfig({
  chains: [chains.hydration, chains.polkadotAssetHubChain], // Include both source and destination chains
  tokens: {
    hdx: {
  ...
  xcmChains: [chains.hydration, chains.polkadotAssetHubChain], // Specify the chains involved for XCM transfers
});
```

### Explanation of Cross-Chain Transfer Configuration

- **chains**: This array should include both the source and destination chains for the transfer.
...
- **xcmChains**: This array should specify the chains that support XCM transfers.
